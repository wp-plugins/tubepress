...
Handles everything related to the options
 ...
 */
function tp_addOptions()
{
    tp_deleteLegacyOptions();
    
    add_option(TP_OPTION_NAME, tp_getNewOptionsArray());
    
    return get_option(TP_OPTION_NAME);
}

/**
 * Compares two arrays to see if they have the same size and the same keys
 * 
 * @param correctArray The array that we compare against
 * @param suspectArray The array we're examining
 */
function tp_deepArrayCheck($correctArray, $suspectArray)
{
    foreach (array_keys($correctArray) as $thisKey) {
        if (!isset($suspectArray[$thisKey])) {
            return false;
        }
        if (!is_array($correctArray[$thisKey])
            && !is_array($suspectArray[$thisKey])) {
                continue;
        }
        if (is_array($correctArray[$thisKey])
            && !is_array($suspectArray[$thisKey])) {
        ...
        if (count($suspectArray[$thisKey]) != count($correctArray[$thisKey])) {
            return false;
        }
        if (!tp_deepArrayCheck($correctArray[$thisKey], 
            $suspectArray[$thisKey])) {
            return false;
        }
    }
    return true;
 ...
 */
function tp_deleteLegacyOptions()
{
 ...
 */
function tp_getNewOptionsArray()
{
    $metaOptions = 
        array(TP_VID_TITLE =>       new TubePressOption(TP_VID_TITLE, 
                                        TP_MSG_VIDTITLE,    '', true),
                                  
              TP_VID_LENGTH =>      new TubePressOption(TP_VID_LENGTH,
                                        TP_MSG_VIDLEN,      '', true),
                                  
              TP_VID_VIEW =>        new TubePressOption(TP_VID_VIEW,
                                        TP_MSG_VIDVIEWS,    '', true),
                                  
              TP_VID_AUTHOR =>      new TubePressOption(TP_VID_AUTHOR ,
                                        TP_MSG_VIDAUTHOR,   '', false),
                                  
              TP_VID_ID =>          new TubePressOption(TP_VID_ID,
                                        TP_MSG_VIDID,       '', false),
                                  
              TP_VID_RATING_AVG =>  new TubePressOption(TP_VID_RATING_AVG,
                                        TP_MSG_VIDRATING,   '', false),
                                  
              TP_VID_RATING_CNT =>  new TubePressOption(TP_VID_RATING_CNT,
                                        TP_MSG_VIDRATINGS,  '', false),
                                  
              TP_VID_UPLOAD_TIME => new TubePressOption(TP_VID_UPLOAD_TIME,
                                        TP_MSG_VIDUPLOAD,   '', false),
                                  
              TP_VID_COMMENT_CNT => new TubePressOption(TP_VID_COMMENT_CNT,
                                        TP_MSG_VIDCOMMENTS, '', false),
                                  
              TP_VID_TAGS =>        new TubePressOption(TP_VID_TAGS,
                                        TP_MSG_VIDTAGS,     '', false),
                                  
              TP_VID_URL =>         new TubePressOption(TP_VID_URL,
                                        TP_MSG_VIDURL,      '', false),
                                  
              TP_VID_THUMBURL =>    new tubePressOption(TP_VID_THUMBURL,
                                        TP_MSG_VIDTHUMBURL, '', false),
                                  
              TP_VID_DESC =>        new TubePressOption(TP_VID_DESC,
                                        TP_MSG_VIDDESC,     '', false));
    $videoSearchOptions = 
        array(TP_SRCH_PLST =>     new TubePressOption(TP_SRCH_PLST,
                                      TP_MSG_SRCH_PLST_TITLE,
                                      TP_MSG_SRCH_PLST_DESC, ''),
                                      
              TP_SRCH_TAG =>      new TubePressOption(TP_SRCH_TAG,
                                      TP_MSG_SRCH_TAG_TITLE,
                                      TP_MSG_SRCH_TAGREL_DESC, ''),
                                      
              TP_SRCH_REL =>      new TubePressOption(TP_SRCH_REL,
                                      TP_MSG_SRCH_REL_TITLE,
                                      TP_MSG_SRCH_TAGREL_DESC, ''),
                                      
              TP_SRCH_USER =>     new TubePressOption(TP_SRCH_USER,
                                      TP_MSG_SRCH_USER_TITLE, '', ''),
                                      
              TP_SRCH_FAV =>      new TubePressOption(TP_SRCH_FAV,
                                      TP_MSG_SRCH_FAV_TITLE, '', ''),
                                      
              TP_SRCH_FEATURED => new TubePressOption(TP_SRCH_FEATURED,
                                      TP_MSG_SRCH_FEATURED_TITLE, '', ''),
                                      
              TP_SRCH_POPULAR =>  new TubePressOption(TP_SRCH_POPULAR,
                                      TP_MSG_SRCH_POPULAR_TITLE, '', ''));
  
    /*  TODO: YouTube's "category" api call doesn't seem to work for now...
              TP_SRCH_CATEGORY => new TubePressOption(TP_SRCH_CATEGORY,
                                      TP_MSG_SRCH_CATEGORY_TITLE, 
                                      'See 
                                      <a href="http://youtube.com/categories">
                                      http://youtube.com/categories</a>', ''));
    */
  
    $searchVariables = 
        array(TP_OPT_SEARCHBY => new TubePressOption(TP_OPT_SEARCHBY, ' ',
                                     '', TP_SRCH_FEATURED),
    ...
    $videoDisplayOptions = 
        array(TP_OPT_VIDSPERPAGE=>  new TubePressOption(TP_OPT_VIDSPERPAGE,
                                        TP_MSG_VIDSPERPAGE_TITLE,
                                        TP_MSG_VIDSPERPAGE_DESC, "20"),
                                        
              TP_OPT_VIDWIDTH =>    new TubePressOption(TP_OPT_VIDWIDTH,
                                        TP_MSG_VIDWIDTH_TITLE,
                                        TP_MSG_VIDWIDTH_DESC, "425"),
                                        
              TP_OPT_VIDHEIGHT =>   new TubePressOption(TP_OPT_VIDHEIGHT,
                                        TP_MSG_VIDHEIGHT_TITLE,
                                        TP_MSG_VIDHEIGHT_DESC, "350"),
                                        
              TP_OPT_THUMBWIDTH =>  new TubePressOption(TP_OPT_THUMBWIDTH,
                                        TP_MSG_THUMBWIDTH_TITLE,
                                        TP_MSG_THUMBWIDTH_DESC, "120"),
              TP_OPT_THUMBHEIGHT => new TubePressOption(TP_OPT_THUMBHEIGHT,
                                        TP_MSG_THUMBHEIGHT_TITLE,
                                        TP_MSG_THUMBHEIGHT_DESC, "90"));
                                        
    $advancedOptions = 
        array(TP_OPT_KEYWORD =>  new TubePressOption(TP_OPT_KEYWORD,
                                     TP_MSG_KEYWORD_TITLE,
                                     TP_MSG_KEYWORD_DESC,
                                     TP_OPTION_NAME),
           
              TP_OPT_TIMEOUT =>  new TubePressOption(TP_OPT_TIMEOUT,
                                     TP_MSG_TIMEOUT_TITLE,
                                     TP_MSG_TIMEOUT_DESC,
                                     "6"),
            
              TP_OPT_DEVID =>    new TubePressOption(TP_OPT_DEVID,
                                     TP_MSG_DEVID_TITLE,
                                     TP_MSG_DEVID_DESC . ' <a href="' .
                                         TP_YOUTUBEDEVLINK . '">' .
                                         TP_YOUTUBEDEVLINK . '</a>',
                                     "qh7CQ9xJIIc"),
    ...
    $videoPlayerLocationOptions =
        array(TP_PLAYIN_NORMAL =>   new TubePressOption(TP_PLAYIN_NORMAL,
                                        TP_MSG_PLAYIN_NORMAL_TITLE, '', ''),
                                        
              TP_PLAYIN_NW =>       new TubePressOption(TP_PLAYIN_NW,
                                        TP_MSG_PLAYIN_NW_TITLE, '', ''),
                                        
              TP_PLAYIN_YT =>       new TubePressOption(TP_PLAYIN_YT,
                                        TP_MSG_PLAYIN_YT_TITLE, '', ''),
    ...
    $videoPlayerMenu = 
        array(TP_OPT_PLAYIN => new TubePressOption(TP_OPT_PLAYIN, 
                                   TP_MSG_PLAYIN_TITLE, '', TP_PLAYIN_NORMAL));
    
    $allOptions = 
        array(TP_OPTS_ADV =>             $advancedOptions,
...
/**
 * Checks to see if the current install has a correctly formatted db row.
 * 
 * @param An array of the options that the user currently has
 * (typically pulled from the db)
 */
function tp_validOptions($options)
{
    if ($options == NULL) {
        return false;
    }
    ...
    return tp_deepArrayCheck(tp_getNewOptionsArray(), $options);
}
?>
