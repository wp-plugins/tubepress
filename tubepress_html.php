...
/**
 * Prints out an embedded video at the top of the gallery. Used in "normal" video
 * playing mode.
 * 
...
EOT;
	$returnVal .= tp_printHTML_embeddedVid($vid->metaValues[TP_VID_ID], $options);
...
/**
 * Prints out video meta information below a video thumbnail. The title and runtime are
 * handled slightly differently than the rest since they look so different.
 * 
 * @param vid A tubepressVideo object of the video in question
 * @param options A tubepressTag object holding all of our options
 * @param css A CSS holder object
 * @param link The attributes of the anchor for the title text. This is generated from
 * 			tp_printHTML_smallVidLinkAttributes()
 */
		...
		if ($options->get_option($metaName)) {
			...
			switch($metaName) {
				...
				case TP_VID_THUMBURL:
					$content .= tp_printHTML_metaLink($option->title, $vid->metaValues[$metaName]);
					break;
				...
				case TP_VID_AUTHOR:
					$content .= $metaName . ': ';
					$content .= tp_printHTML_metaLink($vid->metaValues[$metaName], 'http://www.youtube.com/profile?user=' . $vid->metaValues[$metaName]);
					break;
					
				case TP_VID_COMMENT_CNT:
					$content .= $metaName . ': ';
					$content .= tp_printHTML_metaLink($vid->metaValues[$metaName], 'http://youtube.com/comment_servlet?all_comments&amp;v=' . $vid->metaValues[$metaName]);
					break;
				...
				case TP_VID_TAGS:
					...
					$content .= tp_printHTML_metaLink($vid->metaValues[$metaName], 'http://youtube.com/results?search_query=' . $tags . '&amp;search=Search');
					break;
 ...
 * 
 * @param vidCount How many videos we're supposed to print out per page (+ 1, unless we're on the last page)
 * @param options A tubepressTag object holding all of our options
 * @param css A CSS holder object
 */
function tp_printHTML_pagination($vidCount, $options, $css) {
	...
	$prevText = (($currentPage > 1)? tp_printHTML_paginationLink($url, $currentPage - 1, "< prev") : "&nbsp;");

	/* vidcount will always be one more than what the user wanted, unless we're on the last page */
	$nextText = (($vidCount < $options->get_option(TP_OPT_VIDSPERPAGE))? "&nbsp;" : tp_printHTML_paginationLink($url, $currentPage + 1, "next >"));
...
/**
 * Used in "single video" mode to print out a single video and a "back to gallery" link
 *
 * @param options A tubepressTag object holding all of our options
 * @param css A CSS holder object 
 */
function tp_printHTML_singleVideo($css, $options) {
	...
	$returnVal .= '</div><a href="' . $url->getURL() . '">' . TP_MSG_BACK2GALLERY . '</a>';
	
	return $returnVal;
}

/**
 * The main wrapper method for printing out a single video thumbnail and the meta
 * information for it.
 * 
 * @param vid A tubepressVideo object for which this method will print the thumb
 * @param options A tubepressTag object holding all of our options
 * @param css A CSS holder object
 */
function tp_printHTML_smallvid($vid, $css, $options) {
	...
	$content .= <<<EOT
			<a $link>
				<img alt="{$vid->metaValues[TP_VID_TITLE]}"  src="$thumbSrc" width="$thumbWidth"  height="$thumbHeight"  />
			</a>
...
EOT;
	...
	if ($options->get_option(TP_OPT_PLAYIN) == TP_PLAYIN_THICKBOX) {
		...
		$content .= tp_printHTML_embeddedVid($vid->metaValues[TP_VID_ID], $options) . '</div>';
	}
...
function tp_printHTML_smallVidLinkAttributes($vid, $options) {
	...
	switch ($options->get_option(TP_OPT_PLAYIN)) {
		...
		default:
			return "href=\"#\" onclick=\"javascript:playVideo('" . $id . "', '" . $height . "', '" . $width  . "', '" . htmlspecialchars($vid->metaValues[TP_VID_TITLE])  . "', '" .$vid->metaValues[TP_VID_LENGTH]. "', 'popup', '" . $options->get_option('site_url') . "')\"";
	}
}
